...
from image_validator import is_face_or_cropped_face
...
CLASS_NAMES = ['Blackheads', 'Cyst', 'Nodules', 'Papules', 'Pustules', 'Whiteheads']
...
def _yolo_detections(image: PILImage.Image) -> list[dict[str, object]]:
    ...
    try:
        ...
        results = yolo_model.predict(source=image, verbose=False, conf=0.25, iou=0.4, imgsz=640)
        ...
        for box in result.boxes:
            ...
            cls_name = names.get(class_id, str(class_id))
...
def _yolo_annotated_image(image: PILImage.Image, detections: list[dict]) -> str | None:
    ...
    try:
        ...
        if detections and len(detections) <= 10:
            ...
            cv2.putText(annotated, "Clear skin", (10, int(28 * scale)),
...
async def predict(
    ...
    # ---------------- Relevance validation (real human face/skin) ----------------
    if not is_face_or_cropped_face(image):
        return {"error": "Invalid input: Please upload a real human facial image."}
    ...
    clear_skin_detections = [d for d in detections if d["class"].lower() == "clear_skin"]
    acne_detections = [d for d in detections if d["class"].lower() != "clear_skin"]

    # If YOLO confidently says clear_skin and there are no acne detections, trust it.
    if clear_skin_detections and not acne_detections:
        clear_conf = max(float(d["confidence"]) for d in clear_skin_detections)
        return {
    ...
    # Continue severity/recommendation with acne-only detections.
    detections = acne_detections

    # Classification runs always. If YOLO misses, CNN fallback is used.
    ...
    img_tensor = transform(image_224).unsqueeze(0)
    ...
    confidence_margin = float((top2_probs[0][0] - top2_probs[0][1]).item() * 100)
    ...
    if detection_count == 0:
        # YOLO may miss tiny lesions (e.g., blackheads), so fallback to CNN confidence.
        if confidence_score < 65 or confidence_margin < 12:
            prediction = "Clear skin"
            confidence_score = 0.0
            severity = "No Acne"
            severity_score = 0
        else:
            severity_score = min(int(confidence_score * 0.7), 55)
            severity = "Mild" if severity_score < 30 else "Moderate"
    ...
    if yolo_class and cnn_class and yolo_class != cnn_class:
        ...
        # If YOLO has detections but CNN disagrees, trust YOLO dominant class unless CNN is very strong.
        if confidence_score < 90:
            counts = Counter(yolo_classes)
            dominant_yolo = counts.most_common(1)[0][0]
            prediction = dominant_yolo.replace("_", " ").title()
    ...
    is_clear_skin_result = prediction.lower() == "clear skin"

    if is_clear_skin_result:
        yolo_image = None
        detections = []
        severity = None
        severity_score = None
...
def generate_pdf(payload: PdfReportRequest):
    ...
    if payload.severity_score is not None:
        ...
        if payload.severity_score == 0:
            content.append(Paragraph("Clear skin detected.", styles["Normal"]))
        else:
